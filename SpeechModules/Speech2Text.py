...
class BaiduASR:
    ...
    def _record(self, if_cmu: bool = False, rate=16000):
        ...
        file_name = "./PhoneticTranscription.wav"
    ...
    def speech_to_text(self, audio_path: str = "test.wav", if_microphone: bool = True):
        ...
        if result["err_msg"] != "success.":
            return "log: 语音识别失败：" + result["err_msg"]
        else:
...
class AzureASR:
    ...
    def speech_to_text(self, audio_path: str = "test.wav", if_microphone: bool = True):
        ...
        if speech_recognition_result.reason == speechsdk.ResultReason.RecognizedSpeech:
            print("MoYiJiangNan:{}".format(speech_recognition_result.text))
            return speech_recognition_result.text
        elif speech_recognition_result.reason == speechsdk.ResultReason.NoMatch:
            print("听不出任何声音 :{}".format(speech_recognition_result.no_match_details))
        elif speech_recognition_result.reason == speechsdk.ResultReason.Canceled:
            cancellation_details = speech_recognition_result.cancellation_details
            print("取消语音识别:{}".format(cancellation_details.reason))
            if cancellation_details.reason == speechsdk.CancellationReason.Error:
                print("错误:{}".format(cancellation_details.error_details))
                print("您是否设置了语音资源键和区域值?")
        return None
